def backtest_agressivo(df_test, stop_loss=0.005, take_profit=0.01, trailing_stop=False):
    capital = 10000
    retornos = []
    entradas = []
    saidas = []
    ...
    for i in range(len(df_test) - 10):
        ...
        if (df_test['sinal_compra'].iloc[i] == 1 and df_test['filtros_ok'].iloc[i]):
            ...
            # Garante que haja uma saída mesmo sem TP/SL
            if saida is None:
                preco_atual = df_test['close'].iloc[i + j]
                saida = preco_atual
                tipo_saida = 'Tempo'
            ...
            print(f"🚀 Trade: {df_test.index[i]} ➡ {df_test.index[i + j]} | Tipo: {tipo_saida} | Retorno: {retorno:.4f}")
        else:
            retornos.append(0)
